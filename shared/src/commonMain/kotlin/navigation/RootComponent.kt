...
class RootComponent(
    ...
    private fun createChild(
        ...
        return when(config){
            is Configuration.ScreenA -> Child.ScreenA(ScreenAComponent(
                ...
                onNavigateToScreenB = { text, users ->
                    navigation.pushNew(Configuration.ScreenB(text,users.value))
                }
            ))
    ...
    sealed class Configuration {
        ...
        @Serializable
        data class ScreenB(val text: String, val users: List<RandomUser>) : Configuration()
    }
